...
const descriptionFields = [
  ...
  { label: "What is your total budget for the trip including flights (total for all travellers)?", value: leadData[13] || "" }, // N
  { label: "Number of adults, kids (less than 12 years), and infants (below 2 years) (example: 2A, 1C, 0I)", value: leadData[14] || "" } // O
];
...
async function processCustomerAndLead(connection, leadData) {
  ...
  try {
    ...
    const full_name = leadData[16] || ""; // U column → name
    const phone_number = leadData[17] || ""; // V column
    const email = leadData[15] || ""; // W column
    const city = leadData[18] || ""; // X column → origincity
   const is_qualified = leadData[19] || ""; // AA column → is_qualified
   const	is_quality	   = leadData[20] || ""; // AB column → is_quality
   const is_converted = leadData[21] || ""; // AC column → is_converted
