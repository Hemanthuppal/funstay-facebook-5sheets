...
async function processCustomerAndLead(connection, leadData) {
  ...
  try {
    ...
    const full_name = leadData[20] || ""; // U column → name
    const phone_number = leadData[21] || ""; // V column
    const email = leadData[22] || ""; // W column
    const city = leadData[23] || ""; // X column → origincity
    const is_qualified = leadData[24] || ""; // AA column → is_qualified
   const	is_quality	   = leadData[25] || ""; // AB column → is_quality
   const is_converted = leadData[26] || ""; // AC column → is_converted
